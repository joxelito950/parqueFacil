...
public interface IPlazaDao extends CrudRepository<Plaza, Long> {
    ...
    @Query("select p from Plaza p where p.idParqueadero = :id and p.id = (select h.idPadre from Horario h where h.fechaFin < :fechaInicial)")
    List<Plaza> getPlazasDisponibles(long id, Date fechaInicial);
}
